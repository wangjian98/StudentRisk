...
**局限一：深度架构在事件序列上建模不足。** 主流深度方法 [3,5,6] 直接将 7 维事件序列送入 LSTM、BiLSTM 或 Transformer 等通用序列模型，但这些架构在编程教育场景下表现欠佳（本文实验显示 LSTM-7d / BiLSTM-7d / Attention-7d 的 F1 均 < 0.81，远低于传统 RF-7d 的 0.89）。这是因为标准 RNN / Transformer 缺乏对长序列中选择性状态的有效建模能力，且未利用教育场景的任务结构（problem part）。

**局限二：架构忽视任务结构。** 当前学习分析研究的深度模型架构 [6,7]（LSTM、BiLSTM、Transformer）通常对所有学生一视同仁，使用相同的固定权重。然而，不同 problem part（如第 1 部分 vs 第 7 部分）的行为模式可能截然不同——学生在后期 problem 的挣扎信号与前期的挣扎信号权重应有差异。**任务感知调制**在计算机视觉 [8] 与自然语言处理 [9] 中已展现价值，但在学习分析中尚未被系统探索。
...
| 方法 | 类别 | 描述 |
|---|---|---|
| RF-7d | 树模型 | sklearn RF，**仅 7 个原始事件计数**作为输入（基线） |
| LSTM-7d | 序列 | 单向 LSTM，**7 维事件序列**（one-hot）作为输入 |
| BiLSTM-7d | 序列 | 双向 LSTM，7 维事件序列 |
| Attention-7d | Transformer | 2 层 Transformer Encoder，7 维事件序列 |
...
- 参数量仅 22K（最少），但效果最好
- 较最佳基线 Attention-7d，Accuracy 提升 **+33%**（0.89 vs 0.56），F1(FAIL) 提升 **+11.5%**（0.91 vs 0.80）
- Meta-Mamba-7d（仅 7 维序列）已超越 RF-7d（计数聚合），证明 **原始序列 + 选择性状态空间** 比 **手工聚合** 更强
- 漏报率（FN rate）从 15.3% (RF-7d) 降至 **9.9%**（Meta-Mamba）
...
为更全面地理解模型行为，我们生成了 10 张实验结果图（详见 `outputs/plots/paper/`）：

**图 1 - Meta-Mamba 架构图**（`fig1_architecture.png`）

架构由四部分串联组成：(1) Event Embedding → (2) N=2 层 Mamba Block（含自实现 S6 块 + PreNorm + 残差）→ (3) Task-Aware FiLM 调制 → (4) Masked Mean Pool → Classifier。整条链路上参数共享 task embedding。

**图 2 - 数据集统计**（`fig2_data_stats.png`）

展示 473 名学生的事件分布、失败率（66.4%）、事件类型分布、序列长度分布、problem part 分布等核心统计。

**图 3 - 主结果柱状图**（`fig3_main_results.png`）

直观对比 6 个模型在 Accuracy / Macro-F1 / F1(FAIL) / ROC-AUC 上的表现。Meta-Mamba 全指标领先。

**图 4 - 混淆矩阵网格**（`fig4_confusion_grid.png`）

2×3 网格展示 Meta-Mamba 在 5 折上的混淆矩阵分布。FAILED 类（Class 1）的召回率稳定在 ~90%。

**图 5 - 各类别指标热图**（`fig5_per_class_heatmap.png`）

热图可视化各折各类别 Precision/Recall/F1 稳定性。

**图 6 - 每折 F1 稳定性**（`fig6_per_fold_stability.png`）

Meta-Mamba 在 5 折上的 Macro-F1 稳定性，std 较小表明模型对数据划分不敏感。

**图 7 - 各类别 PR 曲线**（`fig7_per_class_pr_curves.png`）

FAILED 类（positive class）的 PR-AUC=0.97，远高于其他基线。

**图 8 - FOMAML 每任务结果**（`fig8_fomaml_per_task.png`）

每个 problem part 作为 task 的 FOMAML 5-shot 适应结果。多数 task 上 F1 > 0.7，验证任务级共享表征学习成功。

**图 9 - RF-7d 特征重要性**（`fig9_feature_importance.png`）

7 维事件计数中，run 与 submit 的特征重要性最高（合计 >50%），与"提交/运行行为反映学习投入"的直觉一致。

**图 10 - 概念性消融分析**（`fig10_ablation_analysis.png`）

从 RF-7d 基线（0.891）→ 简单 7 维序列模型（0.798-0.808）→ + Mamba 架构（0.890）→ + FiLM + 任务对比（0.905）→ Meta-Mamba 完整（0.914）。**架构升级的边际收益最大的是"事件序列 + 选择性状态空间"（+8.2%），FiLM/对比损失贡献 +2.4%**。
...
**核心发现 1**：架构选择至关重要。简单序列模型（LSTM-7d / BiLSTM-7d / Attention-7d）在 7 维事件序列上表现很差（F1 < 0.81），远低于传统 RF-7d（0.89）。这说明**通用序列架构不能直接迁移到编程教育场景**——它们缺乏对选择性状态的有效建模能力。

**核心发现 2**：选择性状态空间 + 任务感知是关键。Meta-Mamba-7d 仅用 7 维事件序列 + Mamba 架构就达到 F1=0.9111，超过 RF-7d（0.8911）+2.0%，证明 **选择性状态空间 + FiLM 任务调制** 是正确的组合。

**核心发现 3**：原始事件序列 + 强架构 ≈ 时序扩展。Meta-Mamba（11 维含时间间隔/截止距离/题号）仅比 Meta-Mamba-7d 高 0.33% F1 —— 表明 **7 维事件序列 + Mamba 已捕获大部分信号**，时间间隔等连续特征的边际增益有限。
