"""BiLSTM training script: 5-fold × 3 seeds OOF on CS1 dataset."""
import os
import sys
import json
import argparse
...
def run(seeds=(42, 123, 777), n_splits: int = 5, threshold: float = 0.5,
    ...
    if out_dir is None:
        out_dir = os.path.join(_ROOT, 'results', 'bilstm')
    os.makedirs(out_dir, exist_ok=True)

    print(f"[BiLSTM] device={device}", flush=True)
    print(f"[BiLSTM] Loading data + building features ...", flush=True)
    ide_logs, labels_df, y, student_ids = load_dataset()
    X, feat_names = build_features(ide_logs, student_ids)
    print(f"[BiLSTM] X.shape={X.shape}", flush=True)
    ...
    for seed in seeds:
        ...
        print(f"[BiLSTM]   seed={seed} done at {time.time()-t0:.1f}s", flush=True)
    ...
    with open(os.path.join(out_dir, 'config_used.json'), 'w') as f:
        json.dump({'seeds': list(seeds), 'n_splits': n_splits, 'threshold': threshold,
                   'bilstm_cfg': mcfg, 'n_params': n_params}, f, indent=2)

    print(f"\n[BiLSTM] DONE in {elapsed:.1f}s. Overall: "
