"""RF training script: 5-fold × 3 seeds OOF on CS1 dataset.

Run:
    python -m models.rf.train
    python -m models.rf.train --seeds 42 --n-splits 5 --threshold 0.5
"""
import os
import sys
import json
...
def run(seeds=(42, 123, 777), n_splits: int = 5, threshold: float = 0.5,
    ...
    """Run RF 5-fold × N seeds OOF. Returns dict with results."""
    if config is None:
        config = load_config()
    rf_cfg = config.get('random_forest', {})

    if out_dir is None:
        out_dir = os.path.join(_ROOT, 'results', 'rf')
    os.makedirs(out_dir, exist_ok=True)

    print(f"[RF] Loading data ...", flush=True)
    ide_logs, labels_df, y, student_ids = load_dataset()
    print(f"[RF] Building features ...", flush=True)
    X, feat_names = build_features(ide_logs, student_ids)
    print(f"[RF] X.shape={X.shape}, y.shape={y.shape}, "
    ...
    for seed in seeds:
        ...
        for fold_idx_, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(n), y)):
            ...
            fold_records.append({
                'seed': seed, 'fold': fold_idx_,
                **fold_m,
            })
        print(f"[RF]   seed={seed} done at {time.time()-t0:.1f}s", flush=True)
    ...
    with open(os.path.join(out_dir, 'config_used.json'), 'w') as f:
        json.dump({'seeds': list(seeds), 'n_splits': n_splits, 'threshold': threshold,
                   'rf_cfg': rf_cfg}, f, indent=2)

    print(f"\n[RF] DONE in {elapsed:.1f}s. Overall: "
    ...
    print(f"[RF] Results saved to {out_dir}/results.json", flush=True)
    return payload


def main():
    parser = argparse.ArgumentParser(description='Train Random Forest on CS1 dataset')
