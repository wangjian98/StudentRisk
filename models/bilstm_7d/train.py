...
from models.bilstm_7d.data import build_event_sequences_7d
...
def run(seeds=(42, 123, 777), n_splits: int = 5, threshold: float = 0.5,
    ...
    if config is None:
        config = load_config()
    mcfg = config.get('bilstm', {})
    if device is None:
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
    if out_dir is None:
        out_dir = os.path.join(_ROOT, 'results', 'bilstm_7d')
    os.makedirs(out_dir, exist_ok=True)

    print(f"[BiLSTM-7d] device={device}, max_len={max_len}", flush=True)
    print(f"[BiLSTM-7d] Loading data + building 7-dim event sequences ...", flush=True)
    ...
    print(f"[BiLSTM-7d] sequences.shape={sequences.shape}", flush=True)
    ...
    for seed in seeds:
        ...
        print(f"[BiLSTM-7d]   seed={seed} done at {time.time()-t0:.1f}s", flush=True)
    ...
    with open(os.path.join(out_dir, 'config_used.json'), 'w') as f:
        json.dump({'seeds': list(seeds), 'n_splits': n_splits, 'threshold': threshold,
                   'bilstm_cfg': mcfg, 'max_len': max_len, 'n_params': n_params,
                   'feature_dimension': 7}, f, indent=2)

    print(f"\n[BiLSTM-7d] DONE in {elapsed:.1f}s. Overall: "
